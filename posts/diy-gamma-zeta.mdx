...
blurb: 'Sometimes its fun to peek under the hood and see how your computer actually computes things.'
image: '/assets/diy-gamma-zeta/diy-gamma-zeta.webp'
---
...
we use the followinngg reflection formulas to define the value elsewhere: 
...
For complex inputs; we need to realize that the maximum error term increases as the imaginary component; for example, the name estimates that it is required that $n \geq 1.3d + 0.9\Im(x)$, if $d$ digits are required for accuracy. However; for $x$ with small imaginary components, we should use the smallest $n$ that will work:
