...
describe('built site structure', () => {
  ...
  it('renders the post listing', () => {
    expect(existsSync(resolve(dist, 'posts.html'))).toBe(true)
  })

  it('renders individual posts at the expected paths', () => {
    expect(existsSync(resolve(dist, 'posts', 'rendering-fractals.html'))).toBe(true)
    expect(existsSync(resolve(dist, 'posts', 'diy-gamma-zeta.html'))).toBe(true)
  })
...
describe('SEO metadata', () => {
  ...
  const post = () => read('posts/diy-gamma-zeta.html')
